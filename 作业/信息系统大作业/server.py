...
with db_block() as db:
    ...
    ''')
    items = tuple([tuple(row) for row in db])
passwords=dict((y, x) for x, y in items)  
...
async def login_form_page2(request):
    ...
    <form action="/login2" method="post">
        ...
        教工号: <input type="text" name="username" style="margin-top:40px;">
        <p style="text-align:center;"></p>
        密&nbsp;&nbsp;&nbsp码: <input type="password" name="password">
        <input type="submit"  style="background:#7bebfa;margin-top:60px;" value="登录">
    </form>
    <form action="/" method="get">
            <input type="submit" style="background:#7bebfa; margin-top:40px;margin-left: -40px;" value="返回首页">
    </form>
    </body></html>
...
async def login_form_page(request):
    ...
    <form action="/login" method="post"> 
        学号:<input type="text" name="username" style="margin-top:40px;margin-left: 3px;"> 
        <p style="text-align:center;"></p>
        密码: <input type="password" name="password">
        <input type="submit" style="background:#7bebfa;margin-top:60px;"    value="登录">
    </form>
    <form action="/" method="get">
            <input type="submit" style="background:#7bebfa;margin-top:40px;margin-left: -45px;" value="返回首页">
    </form>
    </body></html>
    """, content_type="text/html")
#####管理人员
async def login_form_page3(request):
    ...
    <form action="/login3" method="post">
        ...
        账号: <input type="text" name="username" style="margin-top:40px;margin-left: 3px;">
        <p style="text-align:center;"></p>
        密码: <input type="password" name="password">
        <input type="submit"  style="background:#7bebfa;margin-top:60px;"  value="登录">
    </form>
...
async def student(request):
    ...
    template = jinja_env.get_template('student1.html')
...
async def check_semester(request):
    ...
    with db_block() as db:
        ...
        template = jinja_env.get_template('student2.html')
